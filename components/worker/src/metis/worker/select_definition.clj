...
(defn gen-callback
  [match-m m]
  #(condp = (keyword (stmem/get-val match-m))
     :run   (mu/log ::gen-callback :message "run callback for" :map m)
     :ready (do
              (stmem/set-state-ready (assoc m :message "ready callback")) 
              (stmem/de-register match-m))
     :error (stmem/set-state-error (assoc m :message "error callback for"))))

(defn match-class-fn [s]
  (fn [v] (filterv #(= s (:value %)) v)))

(defn get-classes [m]
  (stmem/get-maps (assoc d :func :cls :no-idx :*)))

(defn get-conds [d]
  (stmem/get-maps (assoc d :func :cond :seq-idx :*)))

(defn get-exch-value [a m]
  (exch/from-path a (get-in m [:value :ExchangePath])))

(defn resolve-cond [a m]
  (let [lft  (get-exch-value a m)
        meth (get-in m [:value :Methode]) ;; the blue one
        rgt  (get-in m [:value :Value])]
    (assoc m :cond-match (cond-match? lft met rgt))))
...
(defn select-definition!
  ...
  in order to avoid the spreading of side effects and easy testing.
    ```" 
  [{cls :DefinitionClass} {mp-id :mp-id}]
  (stmem/set-state-working (assoc m :message "start select-definition"))
  (let [mfn (match-class-fn cls)
        v   (-> {:mp-id mp-id :struct :defins} get-classes mfn)
        v         (filterv #(-> %
                                get-conds
                                resolve-conds
                                conds-match?) v)]
